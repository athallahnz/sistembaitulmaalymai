...
<body onload="window.print()">
    ...
    <div class="info">
        <p>Nama: <strong>{{ $payment->student->name }}</strong></p>
        <p>Kelas: {{ $payment->student->edu_class->name ?? '-' }}</p>
        <p>Tanggal: {{ \Carbon\Carbon::parse($payment->tanggal)->format('d-m-Y') }}</p>
        <p>Jumlah: <strong>Rp {{ number_format($payment->jumlah, 0, ',', '.') }}</strong></p>
        <p>Ket: {{ $keterangan }}</p>
    </div>
